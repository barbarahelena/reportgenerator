...
diversity <- "'מגוון מיקרוביאלי' מתייחס למגוון סוגי החיידקים ושאר המיקרואורגניזמים החיים במעי. רמת גיוון גבוהה מקושרת לרוב לבריאות טובה יותר, בעוד שמגוון נמוך נהוג להיות מקושר למחלות מסוימות. גורמים רבים, כמו תזונה, תרופות ואורח חיים, יכולים להשפיע על המגוון המיקרוביאלי.\n\n"
...
species_explanation <- "תרשים זה מציג את התפלגות מיני החיידקים במיקרוביום המעי שלך. עשרים המינים השכיחים ביותר מוצגים בנפרד, בעוד שאר המינים מאוגדים יחד תחת הקטגוריה \"מינים אחרים\".\n\n."
...
disclaimer <- "דו\"ח זה מבוסס על דגימת צואה בודדת, אשר מספקת תמונת מצב נקודתית של המיקרוביום במעי שלך. הרכב המיקרוביום שלך עשוי להשתנות בעקבות גורמים שונים כגון תזונה, שימוש בתרופות ואורח חיים. לכן, יש לפרש את התוצאות בזהירות, ואין לראות בהן הערכה סופית או מוחלטת של מצב בריאות המעי שלך."
...
create_side_by_side <- function(text_content, plot_filename, caption = NULL) {
  ...
  cat("\\par\\noindent\n")
  cat("\\begin{tabular}{@{}p{0.5\\textwidth}@{\\hspace{0.03\\textwidth}}p{0.45\\textwidth}@{}}\n")
...
for (metric in c("Shannon index", "Species richness")) {
    ...
    div_titletext <- str_c(metric_hebrew, "\\texten{ (", metric, ") }")

    # Create the result text with LaTeX bold formatting and conditional colors
    div_result <- paste0(div_explanations[[metric]],
      "רמת החציון של ", div_titletext, " באוכלוסייה שנבדקה הייתה \\textbf{\\texten{", median_value, "}}, ",
...
create_side_by_side(paste0("\\begin{hebrew}", phylum_explanation, "\\end{hebrew}"),
...
for (taxon in names(keygroups)) {
  ...
  taxa_result <- paste0(
    "\\hebrewfontsf{רמת החציון של }", taxon_title_text, 
