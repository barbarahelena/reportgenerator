...
title: |
  ...
  \arabicfont{تقرير تحليل ميكروبيوم}
  
  \arabicfont{رقم المشارك}: `r params$participant_id`
---

```{r texts, include=FALSE}
# Define all the text content in Arabic
...
diversity <- "\\arabicfont{يشير التنوع الميكروبي إلى تنوع البكتيريا المختلفة والكائنات الحية الدقيقة الأخرى الموجودة في الأمعاء. وغالبًا ما يرتبط التنوع الأعلى بصحة أفضل، في حين يرتبط التنوع المنخفض بأمراض معينة. يمكن للعديد من العوامل، مثل النظام الغذائي والأدوية ونمط الحياة، أن تؤثر على التنوع الميكروبي.}\n\n"
...
phylum_explanation <- "\\arabicfont{يوضح هذا المخطط البياني توزيع المجموعات (الشُعب) البكتيرية الكبرى في ميكروبيوم الأمعاء لديك. وتظهر المجموعات الست العليا الأكثر شيوعًا على نحو منفرد، مع تجميع البكتيريا المتبقية تحت عنوان \"شُعب أخرى\".}\n\n"
...
main_groups <- list(
  ...
  "Pseudomonadota" = "بروتيوباكتيريا شعبة بكتيرية متنوعة تشمل العديد من الأنواع المرتبطة بالالتهابات واضطرابات التمثيل الغذائي. وقد ارتبطت المستويات الأعلى بمرض السكري من النوع 2 والسمنة ومقاومة الأنسولين، في حين ترتبط المستويات الأكثر انخفاضًا بالأنظمة الغذائية الغنية بالألياف وتحسين التحكم في نسبة السكر في الدم."
)

species_explanation <- "\\arabicfont{يوضح هذا المخطط البياني توزيع الأنواع البكتيرية في ميكروبيوم الأمعاء لديك. وتعرض أعلى 20 نوعًا الأكثر وفرة على نحو منفرد، مع تجميع الأنواع المتبقية تحت عنوان \"أنواع أخرى\".}\n\n"
...
taxa_explanations <- list(
  ...
  "Dorea formicigenerans" = "دوريا فورميسيجينيرانس بكتيريا مِعَوية مرتبطة بصحة التمثيل الغذائي. وترتبط المستويات الأعلى من دوريا فورميسيجينيرانس بانخفاض وزن الجسم وتحسين التمثيل الغذائي. تشارك هذه البكتيريا في تخمير الألياف الغذائية، وإنتاج الأحماض الدهنية قصيرة السلسلة مثل بوتيرات، التي لها خواص مضادة للالتهابات وتدعم صحة الأمعاء."
)

bacteria_translations <- list(
  "Bacteroidota" = "\\arabicfont{باكتيروديتس}",
  "Bacillota" = "\\arabicfont{فيرميكيوتس}",
  "Pseudomonadota" = "\\arabicfont{بروتيوباكتيريا}",
  "Desulfovibrionaceae" = "\\arabicfont{ديسلفوفيبريوناسي}",
  "Parabacteroides" = "\\arabicfont{باراباكتيرويديز}",
  "Faecalibacterium" = "\\arabicfont{فيكالبكتريم}",
  "Lactobacillus" = "\\arabicfont{لاكتوباسيلس}",
  "Roseburia" = "\\arabicfont{روزبوريا}",
  "Bacteroides" = "\\arabicfont{باكتيرويديز}",
  "Prevotella" = "\\arabicfont{بريفوتيلا}",
  "Escherichia" = "\\arabicfont{الإشريكيَّة}",
  "Bifidobacterium" = "\\arabicfont{بيفيدوباكتيريوم}",
  "Akkermansia muciniphila" = "\\arabicfont{أكيرمانسيا موسينيفيلا}",
  "Dorea formicigenerans" = "\\arabicfont{دوريا فورميسيجينيرانس}"
)
...
for (metric in c("Shannon index", "Species richness")) {
    ...
    div_result <- paste0(
      ...
      "كان المستوى المتوسط ",
      "ل", metric_arabic,
      " في هذا المجتمع \\texten{", median_value, "}, ",
...
cat("\\subsection*{\\arabicfont{المجموعات البكتيرية الرئيسية}}\n\n")
...
pie1 <- ggplot(pie_phylum, aes(x = "", y = Percentage, fill = label)) +
          ...
          theme(legend.position = "right", legend.text = element_text(size = 14),
                text = element_text(family = "Montserrat"))
ggsave(filename = "figures/report_piephylum.png", plot = pie1, width = 10, height = 3)

create_side_by_side(phylum_explanation,
...
for (taxon in names(keygroups)) {
  ...
  # Then build the complete string with simpler structure
  taxa_result <- paste0(
    "كان المستوى المتوسط ل", taxon_title, " في هذا المجتمع \\textbf{\\texten{", med_formatted, "}}, ",
    "وكان المستوى في عينتك ",
    colored_value, "."
  )
